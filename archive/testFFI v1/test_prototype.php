...
use function CobPhp\add_corresponding;
use function CobPhp\edit_format;
use function CobPhp\initialize;
...
$id3 = new \CobPhp\CobPhpLevel01($id->layout);
...
//================================================================
// 14. WsEmployee — multi-level nested field access
//================================================================
section('14. WsEmployee — nested levels, mixed types, conditions');
...
//================================================================
// 15. move_corresponding — level(01) to level(01), partial match
//================================================================
section('15. move_corresponding — WsEmployee → WsEmployeeCopy (subset)');

$empCopy = $wss->getRecord('WsEmployeeCopy');
...
//================================================================
// 16. WsOrder — OCCURS detail lines via cell()
//================================================================
section('16. WsOrder — OCCURS lines, cell() returns CobPhpLevel');

$order = $wss->getRecord('WsOrder');
...
// cell() returns a CobPhpLevel, not CobPhpLevel01
check('cell() is CobPhpLevel',
    ($order->cell('orderLine', 1) instanceof \CobPhp\CobPhpLevel), true);
check('cell() is NOT CobPhpLevel01',
    ($order->cell('orderLine', 1) instanceof \CobPhp\CobPhpLevel01), false);


//================================================================
// 17. move_corresponding — between cells (CobPhpLevel objects)
//================================================================
section('17. move_corresponding — between cell() results');

$summary = $wss->getRecord('WsOrderSummary');
...
// Summary slot 2 must be untouched (not zeroed — just verify it differs)
...
//================================================================
// 18. add_corresponding — accumulate lines into a summary cell
//================================================================
section('18. add_corresponding — accumulate 3 lines into one cell');
...
//================================================================
// 19. Edited picture formats — edit_format() and auto-MOVE
//================================================================
section('19. Edited picture — edit_format() and auto-MOVE on set()');

// --- edit_format() unit tests ---

// ZZZ,ZZZ,ZZ9.99 — 9 int slots, 2 decimal slots, 14 chars total
...
// ZZZ,ZZZ.99 — 6 int slots, 10 chars total
...
// 9999/99/99 — date insertion (YYYYMMDD → YYYY/MM/DD)
...
// Z9 — small integer with zero suppress
...
// V — virgule virtuelle: decimal digits flow with no printed separator
...
// decimal point is comma — '.' = thousands inserter, ',' = decimal separator
...
// V (virgule virtuelle) is unaffected by DPIC — no printed separator
...
$rpt->set('rlSalary',   75000.00);       // numeric → auto-edited on set()
...
//================================================================
// 20. INITIALIZE — reset fields to category defaults
//================================================================
section('20. INITIALIZE — alphanumeric → SPACES, numeric → ZERO');

// --- INITIALIZE on a level(01) record ---

// WsEmployee already has values from section 14; initialize it and check
initialize($emp);

check('INITIALIZE string → spaces',
    $emp->get('employeeId'), str_repeat(' ', 6));
check('INITIALIZE alpha → spaces',
    $emp->get('employeeGender'), ' ');
check('INITIALIZE packed → zero',
    $emp->get('baseSalary'), 0.0);
check('INITIALIZE binary → zero',
    (int)$emp->get('payGrade'), 0);
check('INITIALIZE float → zero',
    $emp->get('performanceScore'), 0.0);
check('INITIALIZE uint display → zero',
    (int)$emp->get('hireDate'), 0);

// --- INITIALIZE on WsReportLine — numeric-edited fields get MOVE ZERO → mask ---

initialize($rpt);

check('INITIALIZE numeric-edited rlSalary → zero through mask',
    $rpt->get('rlSalary'), edit_format('ZZZ,ZZZ,ZZ9.99', 0));
check('INITIALIZE numeric-edited rlHireDate → zero through mask',
    $rpt->get('rlHireDate'), edit_format('9999/99/99', 0));
check('INITIALIZE rlId (alphanumeric) → spaces',
    $rpt->get('rlId'), str_repeat(' ', 6));
check('INITIALIZE rlMarker → space',
    $rpt->get('rlMarker'), ' ');

// --- INITIALIZE multiple records in one call ---
...
check('INITIALIZE multi: orderId → spaces',
    trim($order->get('orderId')), '');
check('INITIALIZE multi: orderGrandTotal → zero',
    $order->get('orderGrandTotal'), 0.0);
check('INITIALIZE multi: summaryTotal → zero',
    $summary->get('summaryTotal'), 0.0);

// --- INITIALIZE on a cell (CobPhpLevel, not Level01) ---
...
check('INITIALIZE cell: lineQty → zero',
    (int)$order->cell('orderLine', 1)->get('lineQty'), 0);
check('INITIALIZE cell: lineTotal → zero',
    $order->cell('orderLine', 1)->get('lineTotal'), 0.0);
check('INITIALIZE cell: lineCode → spaces',
