...
// CobPhpParser
//
// Parses the level()/occurs()/type declarations from a .cobphp
// file and produces a map of  name → CobPhpLayout.
//
...
//================================================================
final class CobPhpParser
{
    /** @var array<string, CobPhpLayout> */
    ...
    /**
     * Parse a .cobphp source string.
     * Returns map of layoutName => CobPhpLayout.
     *
     * @return array<string, CobPhpLayout>
     */
    ...
    private function consume(string $expected): string
    {
        ...
        if ($tok !== $expected) {
            throw new \RuntimeException(
                "CobPhpParser: expected '$expected', got '$tok' at token {$this->pos}"
            );
        }
        return $tok;
    }
    ...
    private function consumeIdentifier(): string
    {
        $tok = $this->advance();
        if ($tok === '') {
            throw new \RuntimeException("CobPhpParser: expected identifier, got EOF");
        }
        return $tok;
    }
    ...
    private function parseNamespaceBody(): void
    {
        while (! $this->atEnd() && $this->peek() !== '}') {
            ...
            if ($tok === 'level') {
                ...
                $layout = new CobPhpLayout('__standalone_' . $field->name, $field->length);
    ...
    private function parseLevel01(): ?CobPhpLayout
    {
        $this->consume('level');
        $this->consume('(');
        ...
        $layout = new CobPhpLayout($name, $offset, $redefines);
            ...
            if ($tok === 'level') {
                ...
                foreach ($subFields as $sf) {
                    for ($i = 0; $i < $occursMax; $i++) {
                        ...
                        // with occursMax/entrySize so the record can compute
                    ...
                    $fields[] = new CobPhpField(
                        ...
                        occursMax:   $occursMax > 1 ? $occursMax : 1,
                        entrySize:   $occursMax > 1 ? $entrySize : 0,
    ...
    private function parseTypeAndName(?string $forcedName, int $baseOffset): CobPhpField
    {
        // type token — may be  string<10>  or  int<9>  or  float32  etc.
        $typeTok = $this->advance();

        [$type, $digits, $decimals, $length, $flags] = $this->resolveType($typeTok);
        ...
        while (in_array($this->peek(), ['binary','comp','packed','native','sync',
            ...
            if ($mod === 'packed') {
                ...
                $length = (int)ceil(($digits + 1) / 2);   // BCD storage
        ...
        // skip optional  = value  initialiser
        if ($this->peek() === '=') {
            $this->advance();
            $this->advance(); // value token
        }
    ...
    private function resolveType(string $tok): array
    {
        ...
        if (preg_match('/^edited$/', $tok)) {
            // next token is <"mask">  or  <'mask'>
            // for simplicity, read until > and count edit chars
            $mask = '';
            if ($this->peek() === '<') {
                $this->advance();
            ...
            return [FieldType::Display, 0, 0, $len, $flags];
        }
    ...
    //------------------------------------------------------------
    private function parseStandalone(): CobPhpField
    {
