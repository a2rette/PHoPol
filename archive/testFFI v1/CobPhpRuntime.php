...
final class CobPhpRuntime
{
    ...
    public static function editFormat(string $mask, mixed $value, bool $decimalPointIsComma = false): string
    {
        ...
        $firstSig = $intSlots - 1;                      // default: last position (all-zero)
...
function move_corresponding(CobPhpLevel $src, CobPhpLevel $dst): void
{
    CobPhpRuntime::moveCorresponding($src, $dst);
}

function add_corresponding(CobPhpLevel $src, CobPhpLevel $dst): void
{
...
{
    return CobPhpRuntime::editFormat($mask, $value, $decimalPointIsComma);
}

// INITIALIZE accepts one or more levels (mirrors COBOL: INITIALIZE A B C)
function initialize(CobPhpLevel ...$levels): void
{
