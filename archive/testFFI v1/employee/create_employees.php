...
foreach ($rows as $row) {
    ...
    $emp->set('employeeId',       $id);
    $emp->set('employeeName',     $name);
    $emp->set('employeeGender',   $gender);
    $emp->set('employeeStatus',   $status);
    $emp->set('baseSalary',       $base);
    $emp->set('bonus',            $bonus);
    $emp->set('totalComp',        $total);
    $emp->set('payGrade',         $grade);
    $emp->set('hireDate',         $hire);
    $emp->set('reviewDate',       $review);
    $emp->set('yearsService',     $years);
    $emp->set('performanceScore', $score);
...
fclose($fh);
$recSize = $emp->layout->totalLength;
echo "\nFile: $outPath  (" . count($rows) . " records × {$recSize} bytes = " . (count($rows) * $recSize) . " bytes)\n";
