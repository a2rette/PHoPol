...
//================================================================
// CobPhpLevel — base class for any level(n) group item
//
...
//
// CobPhpLevel01 (level 01) extends this and adds buffer
...
class CobPhpLevel
{
    ...
    // short field names, then returns a CobPhpLevel view
    // over that slice of the shared buffer.
    //------------------------------------------------------------
    public function cell(string $groupPrefix, int $idx): CobPhpLevel
    {
        $subFields = [];
        $entrySize = 0;
        $groupBase = 0;
        ...
        $subLayout = new CobPhpLayout("{$groupPrefix}[{$idx}]", $entrySize, null, $this->layout->decimalPointIsComma);
        foreach ($subFields as $sf) {
            $shortName = (string)preg_replace('/^.*\[\*\]_/', '', $sf->name);
            $subLayout->addField(new CobPhpField(
                name:        $shortName,
        ...
        return new CobPhpLevel($subLayout, $this->buffer, $cellBaseOffset);
    }
    ...
    private function isNumericCategory(CobPhpField $field): bool
    {
    ...
    protected function decode(CobPhpField $field, int $offset): mixed
    {
    ...
    protected function encode(CobPhpField $field, int $offset, mixed $value): void
    {
        match($field->type) {
    ...
    private function encodeDisplay(CobPhpField $field, int $offset, mixed $value): void
    {
        ...
        if ($field->editMask !== '' && !is_string($value)) {
            $value = CobPhpRuntime::editFormat($field->editMask, $value, $this->layout->decimalPointIsComma);
        }
